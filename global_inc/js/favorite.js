...
function LoadFavoriteFormData(sFormData, blnReload)
{
	...
	xmlhttp.open("http://www.nabj.org/POST", "http://www.nabj.org/global_engine/ajax/favorite_form.asp", false);
	...
	if(jsonData.FavoriteID == 0){
		...
		if (objectExists(document.getElementById('FavoriteOptsLinkImg')))
		{
			...
			document.getElementById('FavoriteOptsLinkImg').src = 'http://www.nabj.org/global_graphics/icons/fav.gif';
...
function FavoriteEditor_Submit(fFavoriteEditor)
{
	...
	if (FavoriteEditor_IsValid(sFormData))
	{
		...
		sendSimpleHttpRequest("http://www.nabj.org/global_engine/ajax/favorite_process.asp", FavoriteCallbackHandler, sFormData, xmlhttp);		
...
function FavoriteEditor_btnDelete_OnClick(sender)
{
	...
	if (objectExists(document.getElementById('FavoriteOptsLinkImg')))
	{
		...
		document.getElementById('FavoriteOptsLinkImg').src = 'http://www.nabj.org/global_graphics/icons/fav_off.gif';
