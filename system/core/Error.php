...
class SJB_Error
{
	...
	private static $maxRuntimeErrorLevel = null;
	...
	public $errorCode = array (
		// LISTING ERRORS
			...
			'WRONG_LISTING_ID_SPECIFIED'	=> 'There is no listing in the system with the specified ID',
	 ...
	 * @param integer $line
	 */
	public static function errorHandler($intLevel, $message = NULL, $file = NULL, $line = NULL)
	{
		...
		if ($errorMode == 'debug') {
			...
			$types = array(
				...
				"Notice"      => array("E_NOTICE", "E_USER_NOTICE", "E_DEPRECATED", "E_USER_DEPRECATED"),
			);
			// Textual error type.
			$type = "Unknown error";
			...
			echo  $prefix . "<b>{$type}</b>:  {$message} in <b>{$file}</b> on line <b>{$line}</b><br />" . $suffix;
		}
	...
	public static function fatalErrorHandler($output)
	{
		...
		SJB_Error::writeToLog(array( array('level' => 'E_ERROR', 'message' => $msg, 'file' => $file, 'line' => $line)));
	...
	public static function getErrorContent()
	{
		if ( !empty(self::$runtimeErrors)) {
			...
			if ($errLvl == 'E_NOTICE' || $errLvl == 'E_WARNING') {
				return '<div class="main"><p class="error">Done with errors on the page</p></div>';
			}
		}
	...
	public static function writeToLog($errors)
	{
		...
		if (is_array($errors)) {
			foreach ($errors as $error) {
				...
				$errorsString .= "<p><b>{$caption}:</b> {$error['message']} in <b>{$error['file']}</b> on line <b>{$error['line']}</b></p>\n";
			}
		} else {
			$errorsString = $errors;
		}
