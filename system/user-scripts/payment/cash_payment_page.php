...
$alreadyOpenedInvoice = SJB_OpenInvoiceManager::getOpenInvoiceByPaymentSID($payment_id);

if (!$alreadyOpenedInvoice) {
	...
	//		01-02-2018 - SJB_PaymentManager::endorsePayment($payment_id);
			SJB_PaymentManager::verifyPaymentJobAutoActivate($payment_id);
		}
	
	    $tp->assign('payment_id', $payment_id);
	    $tp->assign('item_name', $_REQUEST['item_name']);
	    
	    if(  isset($product_info['membership_plan_id'])  ) {
	    $tp->assign('plan_id', $product_info['membership_plan_id']);
	    }
...
// $tp->assign("send_email", $send_email);
	...
	$errors['ALREADY_OPENED_INVOICE'] = true;
}
