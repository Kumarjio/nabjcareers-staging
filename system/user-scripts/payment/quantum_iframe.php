...
function _quantumilf_http_post($host, $path, $data, $port = 80) {
	...
	$response = '';
	if ( (PHP_VERSION >= 4.3) && ($fs = @fsockopen('ssl://secure.quantumgateway.com', 443, $errno, $errstr, 30)) ) {
		...
		$ch = curl_init();
		$curl_url = 'https://secure.quantumgateway.com' . $path;
...
function quantumilf_getCode($API_Username, $API_Key, $width, $height, $amount='0', $id='0', $custid='0', $method='0', $addtoVault='N', $skipshipping='N') {
	...
	$random = (int)$random;
	$response = _quantumilf_http_post("secure.quantumgateway.com", "/cgi/ilf_authenticate.php",
			array (
	...
	if (is_array($response)) {
		if ($response[1] != 'error') {
			...
			var tposturl = "quantum_iframe.php?cachebuster=" + randomnumber;
...
if (isset($_POST['ajax']))  {
	if ($_POST['ajax'] == "true") {
		$response = _quantumilf_http_post("secure.quantumgateway.com", "/cgi/ilf_refresh.php",
				array (
						'ip' => $_POST['ip'],
						'k' => $_POST['k']
				), 443
