...
$handle = fopen($script_path."sitemap.xml", "w");
...
foreach ($list_of_pages as $page) {
	if ($page['uri'] == '/display-job/') {
		...
		$found_listings_sids = searchListings($request, 'Job');
...
<priority>1</priority>
	...
	elseif ($page['uri'] == '/display-resume/') {
		...
		$found_listings_sids = searchListings($request, 'Resume');
		foreach ($found_listings_sids as $sid) {
			$listing_info = SJB_ListingManager::getListingInfoBySID($sid);
			$title = htmlspecialchars($listing_info['Title']);
			$title = $sid."/".preg_replace("/[\\/\\\:*?\"<>|%#$\s]/", "_", $title).".html";
...
function searchListings($requested_data, $listing_type_id) 
{
	...
	$found_listings_sids = getListingSidCollectionFromRequest($requested_data, $listing_type_sid, $criteria_saver);
	return $found_listings_sids;
}

function getListingSidCollectionFromRequest($requested_data, $listing_type_sid, $criteria_saver) 
{		
