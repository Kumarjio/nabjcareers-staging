...
if (SJB_UserManager::isUserLoggedIn()) {
	...
	switch ($action) {
		...
		case 'save':
			...
			if (empty($search_name['equal'])) {
				...
				$emailFrequency = $emailFrequency?'&email_frequency='.array_pop(array_pop($emailFrequency)):'';
				...
				SJB_HelperFunctions::redirect(SJB_System::getSystemSettings('SITE_URL') . "/save-search/?alert=true&url=" . SJB_System::getURI() . "&action=save&search_name=$search_name" . "&searchId=" . $searchResultsTP->searchId.$emailFrequency);
			}
		break;
		...
		case 'edit';
			if (isset($_REQUEST['id_saved'])) {
				...
				if (empty($search_name['equal']))
					...
					unset($_REQUEST['name']);
					unset($_REQUEST['email_frequency']);
					$emailFrequency = $emailFrequency?array_pop(array_pop($emailFrequency)):false;
					$searchResultsTP = new SJB_SearchResultsTP($_REQUEST, $listing_type_id);
					$tp = $searchResultsTP->getChargedTemplateProcessor();
				...
				if (!empty($errors)) {
					...
					SJB_HelperFunctions::redirect(SJB_System::getSystemSettings('SITE_URL').SJB_System::getURI()."?alert=is_update");
				}
			}
		break;
	...
	switch (SJB_Request::getVar("alert")) {
		case 'added':		$tp->assign("alert_added", 'added');	break;
		case 'is_update':	$tp->assign("alert_update", 'update');	break;
	}
