...
class SJB_Rating extends SJB_Object
{
	...
	function setRaiting($rate, $listing_sid, $user_id)
	{
	...
	function getRatingListing($listing_sid)
	{
		return SJB_DB::query("SELECT id, ctime, r.vote AS vote, u.username , u.email , u.user_group_sid FROM `rating` as r , users as u WHERE u.sid=user_id AND  r.listing_id = ?n ORDER BY r.ctime DESC", $listing_sid);
	}
	...
	function deleteRatingByUserSID($user_id)
	{
		return SJB_DB::query("DELETE FROM `rating`  WHERE user_id = ?n ", $user_id);
	}
	
	function getListingSIDByRatingSID($rating_id)
	{
