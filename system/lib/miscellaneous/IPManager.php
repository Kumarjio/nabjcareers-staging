...
class SJB_IPManager
{
	...
	{
		$sql = SJB_DB::query("SELECT `value` FROM `banned_ips` WHERE `value`=?s", $ip);
		return $sql?array_pop(array_pop($sql)):false;
	}
	
	public static function getBannedIPByID($id)
	{
		$sql = SJB_DB::query("SELECT `value` FROM `banned_ips` WHERE `id`=?s", $id);
		return $sql?array_pop(array_pop($sql)):false;
	}
}