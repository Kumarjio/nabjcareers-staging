...
class SJB_TemplateEditor {
	...
	{
		$path_to_template = SJB_TemplatePathManager :: getAbsoluteTemplatePath($theme_name, $module_name, $tpl_name, $this->access_class);
	...
	{
		$template_path = SJB_TemplatePathManager :: getAbsoluteTemplatePath($theme_name, $module_name, $tpl_name, $this->access_class);
	 ...
	 * @return unknown
	 */
	function getTemplateList($module_name, $theme_name) {
		$tpl_path = SJB_TemplatePathManager :: getAbsoluteTemplatePath($theme_name, $module_name, '', $this->access_class);
		$system_path = SJB_TemplatePathManager :: getAbsoluteTemplatePath(SJB_System::getSystemSettings('SYSTEM_TEMPLATE_DIR'), $module_name, '', $this->access_class);
		$template_list = array();
	...
	function isTemplateWriteable($module_name, $theme_name, $template_name)
	{
		...
		$path_to_file = SJB_TemplatePathManager :: getAbsoluteTemplatePath($theme_name, $module_name, $template_name, $this->access_class);
	...
	function getThemeList()
	{
		...
		$themes = array ();
		$templates_path = SJB_TemplatePathManager::getAbsoluteTemplatesPath ($this->access_class);
	...
	//------------------------------------------------------------------------
	// deletes theme
	function deleteEntireTheme($theme_name)
	{
		if ($theme_name == 'default')
			return false;

		SJB_Filesystem :: delete(SJB_TemplatePathManager::getAbsoluteThemePath ($theme_name, $this->access_class));
		SJB_Filesystem :: delete(SJB_TemplatePathManager::getAbsoluteThemeCachePath ($theme_name));
		return true;
	}

	//------------------------------------------------------------------------
	// cpoies theme
	function copyEntireTheme($from, $to)
	{
		$from_path = SJB_TemplatePathManager::getAbsoluteThemePath ($from, $this->access_class);
		$to_path = SJB_TemplatePathManager::getAbsoluteThemePath ($to, $this->access_class);
		SJB_Filesystem :: copy($from_path, $to_path);
		SJB_Filesystem :: mkpath(SJB_TemplatePathManager::getAbsoluteThemeCachePath ($to));
	}
        ...
        {
            $path_to_template = SJB_TemplatePathManager :: getAbsoluteTemplatePath($theme_name, $module_name, $tpl_name, $this->access_class);
            $newPath_to_template = SJB_TemplatePathManager :: getAbsoluteTemplatePath($theme_name, $newModuleName, $newTplName, $this->access_class);
