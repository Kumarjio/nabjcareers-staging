...
class SJB_Banners
{
	 ...
	 * @return integer
	 */
	function getBannerIdByGroupID($groupID)
	{
		$result = array_pop(SJB_DB::query('SELECT b.id
										FROM banners as b
	 ...
	 * @return integer
	 */
	function getActiveBannerIdByGroupID($groupID)
	{
	...
	function getNumberBannersDisplayAtByGroupID($groupID)
	{
		$number_banners_display_at_once = SJB_DB::query('SELECT `number_banners_display_at_once` FROM `banner_groups` WHERE id = ?s', $groupID);
		return $number_banners_display_at_once ? array_pop(array_pop($number_banners_display_at_once)) : 1;
	}
	...
	{
		$res = SJB_DB::query("SELECT b.*, bg.id as groupID FROM `{$this->bannersTable}` as b INNER JOIN banner_groups as bg ON (bg.sid = b.groupSID) WHERE b.groupSID = ?n", $groupSID);
		foreach ($res as $key => $val) {
			if ($val['show'] > 0)
	...
	function getBannersPath()
	{
		...
		$bannersPath = $siteUrl . '/files/banners';
		return $bannersPath;
	}
	 ...
	 * @return boolean
	 */
	function addBanner($title, $link, $imagePath, $sx, $sy, $type, $active, $groupSID = '', $params, $start, $end)
	{
		$link = preg_replace("|\.\./|u", '', $link);
		SJB_DB::query("INSERT INTO `{$this->bannersTable}`
								SET  `title` = ?s, `link` = ?s, `image_path` = ?s, `type` = ?s, `width` = ?n, `height` = ?n, `active`=?n, `groupSID`=?n, openBannerIn =?s, bannerType = ?s, code = ?s, `start_date`=?n, `end_date`=?n",
			$title, $link, $imagePath, $type, $sx, $sy, $active, $groupSID, $params['openBannerIn'], $params['bannerType'], $params['code'], $start, $end);
		return true;
	}
	 ...
	 * @return boolean
	 */
	function updateBanner($id, $title, $link, $imagePath, $sx, $sy, $type, $active, $groupSID = '', $params, $start, $end)
	{
		$link = preg_replace("|\.\./|u", '', $link);
		SJB_DB::query("UPDATE `{$this->bannersTable}`
								SET  `title` = ?s, `link` = ?s, `image_path` = ?s, `type` = ?s, `width` = ?n, `height` = ?n, `active` = ?n, `groupSID` = ?n, openBannerIn =?s, bannerType = ?s, code = ?s, `start_date`=?s, `end_date`=?s  
								WHERE `id` = ?n",
			$title, $link, $imagePath, $type, $sx, $sy, $active, $groupSID, $params['openBannerIn'], $params['bannerType'], $params['code'], $start, $end, $id);
		return true;
	}
	 ...
	 * @return boolean
	 */
	function incrementShowCounter($bannersIDs)
	{
		foreach ($bannersIDs as $bannerID) {
			$id = $bannerID['id'];
			SJB_DB::query('UPDATE `banners` SET `show` = `show`+1 WHERE `id` = ?n LIMIT 1', $id);
		}
		return true;
	}
	...
	function deleteBanner($id)
	{
		...
		if (empty($bannerInfo['image_path'])) {
			$this->bannersError = 'No files to delete';
		...
		(
			...
			(
				...
				'order' => 6,
							...
							/* 07-06-2014 Start-End dates mod for Banners**/
							array
							(
									'id' => 'start_date',
									'caption' => 'Start Date',
							...
							/* END - 07-06-2014 Start-End dates mod for Banners**/
