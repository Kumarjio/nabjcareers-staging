...
class SJB_UserDetails extends SJB_ObjectDetails
{
	...
	{
		$details_info = SJB_UserDetails::getDetails($user_group_sid);
	...
	public static function getDetails($user_group_sid)
	{
		$details =  array (
			    array (
					'id'			=> 'featured',
				...
				array (
					'id'		=> 'sendmail',
					'caption'	=> "Send mailings", // previous: "Don't send mailings"
					'type'		=> 'boolean',
					'length'	=> '1',
					'is_required'=> false,
					'is_system'=> true,
		...
		if ((SJB_UserGroupManager::isUserEmailAsUsernameInUserGroup($user_group_sid)) ||
			(class_exists ('SJB_SocialPlugin') && SJB_UserManager::getCurrentUserSID() && SJB_SocialPlugin::getProfileSocialID(SJB_UserManager::getCurrentUserSID())))
			$userFields = array (
						 array (
							'id'		 => 'username',
							'caption'	 => 'User name', 
		...
		if (SJB_System::getSettingByName('registrationCaptcha') == 1 && SJB_System::getURI()== '/registration/') {
