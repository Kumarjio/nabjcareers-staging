...
class SJB_PaymentInfoSearcher extends SJB_ObjectInfoSearcher {
	...
	function SJB_PaymentInfoSearcher($limit = false, $sorting_field = false, $sorting_order = false, $inner_join = false) {
		parent::SJB_ObjectInfoSearcher('payments');
		$this->limit = $limit;
	...
	function getObjectInfo($sorting_fields)
	{
        ...
        $sql_string = $SearchSqlTranslator->buildSqlQuery( $this->criteria, $this->valid_criterion_number, $sorting_fields, $this->inner_join );
      	$totalPriceSql = str_replace('`payments`.`sid` as `object_sid`', 'ifnull(sum(`payments`.price), 0) as `payment`', $SearchSqlTranslator->buildSqlQuery( $this->criteria, $this->valid_criterion_number, $sorting_fields));
		$where = '';
