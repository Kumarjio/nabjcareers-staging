...
require_once "AuthNetAIMDetails.php";

class SJB_AuthNetAIM extends SJB_PaymentGateway

{

    function SJB_AuthNetAIM($gateway_info = null)
...

    {
			$properties = $this->details->getProperties();
			$api_login_id 				=	$properties['authnet_api_login_id']->getValue();
			$_SESSION['api_login_id']	=	$api_login_id;	
			$transaction_key 			= 	$properties['authnet_api_transaction_key']->getValue();
			$_SESSION['transaction_key']=	$transaction_key;
			return SJB_System::getSystemSettings('SITE_URL').'/payment-page-authorize/?payment_sid='.$_GET['payment_id'];
	}
