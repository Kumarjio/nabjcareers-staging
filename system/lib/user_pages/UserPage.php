...
class SJB_UserPage{
	...
	function extractPageData($request) {
	...
	function unserializeParameters($parameters) {
		if (empty($parameters)) {
		    return null;
		}
		else {
    ...
    function isDataValid(){
        if ( $this->pagedata['uri'] == '') {
	   	...
	   	foreach($modules as $module) { 
	   		if ( $module == $this->pagedata['module'] ) { $is_exist = 1; break; }		
	   	}
		if ( $is_exist == 0 ) {
			...
			return false;
		  	...
		  	foreach($functions as $func) { 
		   		if ( $func == $this->pagedata['function'] ) { $is_exist = 1; break; }		
		   	}
			if ( $is_exist == 0 ) {
