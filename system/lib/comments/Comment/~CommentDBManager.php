...
class SJB_CommentDBManager extends SJB_ObjectDBManager
{
	function saveComment($comment)
	{
		...
		SJB_DB::query("UPDATE `comments` SET `added` = NOW() WHERE `sid` = ?n", $comment->getSID());
	...
	function getCommentsNumToListing($listing_sid)
	{
	...
	{
		$ids = SJB_DB::query("SELECT `sid` FROM `comments` WHERE `listing_id` = ?n AND `disabled` <> '1'", $listing_sid);
	...
	function getCommentsToListing($listing_sid)
	{
		...
		$comments = SJB_CommentDBManager::getCommentsInfoBySIDCollection($sid_collection);
		return $comments;
	}
