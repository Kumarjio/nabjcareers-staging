...
class SJB_ListingDetails extends SJB_ObjectDetails
{
	...
	public static function getDetails($listing_type_sid, $pageID = 0)
	{
		...
		foreach ($details as $key => $detail) {
			...
			if ($detail['id'] == 'anonymous' || $detail['id'] == 'screening_questionnaire')
		...
		if ($isPageLast) {
			...
			// Special property, used for JobG8 integration
			// not empty only for JobG8 listings
			$listingCompanyDetails = array(
					array (
						'id'			=> 'company_name',
			...
			$details = array_merge($details, $listingCompanyDetails);
				
			if (SJB_System::getSettingByName('postJobCaptcha') && strpos(SJB_System::getURI(), '/add-listing/') === 0) {
