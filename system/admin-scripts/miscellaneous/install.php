...
class Requirements {
	...
	function Requirements () {
		$this -> requirement_phpversion = "4.3";
...
class Form {
	...
	function Form ($params) {
		$this -> formfields = array (
			'dbhost' => 'localhost', 'dbname' => '', 'dbuser' => '', 'dbpswd' => '',
			'adminname' => '', 'adminpswd' => '',
			'system_email' => '',
			'ftphost' => 'localhost', 'ftpdir' => getcwd(), 'ftpuser' => '', 'ftppswd' => '',
		);
	...
	function check_mysql_connection () {
		...
		$connection = @mysql_connect($this -> dbhost, $this -> dbuser, $this -> dbpswd);
		if ($connection === false) {
			logging (ERROR, "Cannot establish connection to MySQL server: #".mysql_errno ()." - ".mysql_error ());
			return false;
		}
		if (mysql_select_db ($this -> dbname) === false) {
			logging (ERROR, "Cannot find DB: #".mysql_errno ()." - ".mysql_error ());
			return false;
		}
		mysql_close ();
		return true;
...
class Installation {
	...
	function Installation ($params) {
		...
		$this->writable_dirs = array('files/files', 'files/video', 'files/pictures', 'temp', 'languages', "./");
	...
	function define_baseurl () {
		...
		return $this -> baseurl = 'http://{$_SERVER[\'HTTP_HOST\']}' . $p['dirname'];
	}
	...
	function set_charset () {
		...
		$link = @mysql_connect ($this -> dbhost, $this -> dbuser, $this -> dbpswd);
		if ($link === false) {
			logging (ERROR, "Cannot establish MySQL connection: #".mysql_errno ()." - ".mysql_error ());
			return false;
		}
		...
		if ($this -> sql_version_above41())
			if ($this -> current_character_set() != 'utf8')
				if ($this -> is_utf8_available())
	...
	function create_base_tables () {
		...
		foreach ($commands as $command) {
			...
			if (!mysql_query ($command['query'])) {
				logging (ERROR, "Cannot execute MySQL query: #".mysql_errno ()." - ".mysql_error ());
				return false;
			}
		}
		mysql_close ();
	...
	function forming_config_files () {
		...
		$www_config_vars = array (
			'DBHOST' => '', 'DBNAME' => '', 'DBUSER' => '', 'DBPASSWORD' => '', 'SITE_URL' => '',
		);

		$www_config_vars['DBHOST'] = $this -> dbhost;
		$www_config_vars['DBNAME'] = $this -> dbname;
		$www_config_vars['DBUSER'] = $this -> dbuser;
		$www_config_vars['DBPASSWORD'] = $this -> dbpswd;
		$www_config_vars['SITE_URL'] = $this -> baseurl;
		if (!empty ($this -> mysql_charset))
			$www_config_vars['MYSQL_CHARSET'] = $this -> mysql_charset;

		$www_config_vars['ADMIN_SITE_URL'] = $this -> baseurl.'/admin';
		$www_config_vars['USER_SITE_URL'] = $this -> baseurl;

		$www_config_file_content = "<?php\r\n\r\n"
									."return array(\r\n";
		foreach ($www_config_vars as $key => $value) {
			$www_config_file_content .= '"'.$key.'" => "'.str_replace("\'", "'", $value).'",'."\r\n";
		}
	...
	function write_admin_db_config () {
		...
		if (@mysql_connect ($this -> dbhost, $this -> dbuser, $this -> dbpswd) === false) {
			logging (ERROR, "Cannot establish connection to MySQL: #".mysql_errno ()." - ".mysql_error ());
			return false;
		}
		if (!@mysql_select_db ($this -> dbname)) {
			logging (ERROR, "Cannot find DB: #".mysql_errno ()." - ".mysql_error ());
		...
		unset ($sql);
		$sql = "INSERT INTO `administrator`(`username`, `password`) VALUES('".mysql_real_escape_string ($this -> adminname)."', '". md5(mysql_real_escape_string ($this -> adminpswd)) ."')";
		if (mysql_query ($sql) === false) {
			logging (ERROR, "Cannot execute MySQL query: #".mysql_errno ()." - ".mysql_error ());
			return false;
		}
		
		$sql = "UPDATE settings SET value = '".mysql_real_escape_string($this->system_email)."' WHERE name = 'system_email'";
	...
	function write_www_htaccess () {
		...
		$rewrite_base_path = '';
		$url_params = parse_url ($this -> baseurl);
		...
		foreach ($strings as $index => $cstr) {
			if (strpos ($strings[$index], "RewriteBase") !== false)
				$strings[$index] = "RewriteBase ".$rewrite_base_path."/\r\n";
	...
	function sql_version_above41()
	{
		...
		$version_num = $arr['Value'];
		$parts = split('[/.-]', $version_num);
	...
	function is_utf8_available()
	{
		...
		while($arr = mysql_fetch_assoc($res))
			$result = $result || $arr['Charset'] == 'utf8';
		return $result;
	}
	...
	{
		$sql = "alter database `".mysql_real_escape_string($dbname)."` character set '".mysql_real_escape_string($charset)."';";
		if(!mysql_query($sql))
			return false;
		return true;
	}
