...
if ($listing_type_sid) {
	...
	switch ($action) {
		case 'new':
			...
			$form_submitted = (isset($submit) && $submit == 'Save');
			if ($submit) {
		...
		case 'edit':
			$template = 'input_page_form.tpl';
			$saveFields = SJB_Request::getVar('saveFields', false);
			...
			switch ($field_action) {
				...
				case 'remove':
					$field_sid = SJB_Request::getVar('field_sid', null);
					SJB_PostingPagesManager::removeFieldFromPade($field_sid, $listing_type_sid);
					break;
				case 'move':
					$field_sid = SJB_Request::getVar('field_sid', null);
				...
				case 'save_order':
					...
					//echo "pageSID: $pageSID<br>";
					//SJB_HelperFunctions::dd($item_order); exit;
