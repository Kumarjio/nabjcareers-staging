...
<form name="form1" method="post">
	...
	<fieldset>
		...
		<table>
			<tr><td colspan="2"><input type="hidden" name="ID" value="{$user_page.ID}" class="text" /></td></tr>
			<tr>
				<td>URI</td>
				<td><input type="text" name="uri" value="{$user_page.uri}" class="text" /></td>
			</tr>
			<tr>
				<td>Pass parameters via URI</td>
				<td><input type="checkbox" name="pass_parameters_via_uri" {if $user_page.pass_parameters_via_uri} checked {/if} class="text" /></td>
			</tr>
			<tr>
				<td>Title</td>
				<td><input type="text" name="title" value="{$user_page.title}" class="text" /></td>
			</tr>
			<tr>
				<td>Template</td>
				<td><input type="text" name="template" value="{$user_page.template}" class="text" /></td>
			</tr>
			<tr>
				<td>Module</td>
				<td>
					<select size="1" name="module" id="modules" class="list" onchange="loadFunctionsForModule(this.form)" LANGUAGE="Javascript">	    
					    ...
					    {foreach from=$LIST_MODULES key=KEY_MOD item=VALUE_MOD}  				    
						    {if $VALUE_MOD == $user_page.module} <option selected> {$VALUE_MOD} </option> 
						    {else} <option> {$VALUE_MOD} </option>  	    	
						    {/if}				
					  	{/foreach}
					</select>
				</td>
				...
				<td>
					<select size="1" name="function" id="functions" class="list" onchange="loadParamsForFunction()" LANGUAGE="Javascript">	    				
					    ...
					    {foreach from=$LIST_FUNCTIONS[$user_page.module] key=KEY_FUNC item=VALUE_FUNC}  				    		    	
						    {if $VALUE_FUNC == $user_page.function} <option selected> {$VALUE_FUNC} </option> 
						    {else} <option> {$VALUE_FUNC} </option>  	    	
						    {/if}				
					  	{/foreach}
				</td>
			</tr>
								...
								{foreach from=$VALUE_ARRAY_PARAM key=KEY_PARAM item=VALUE_PARAM}			
								  ...
								  {foreach from=$a_params key=key_a item=value_a}			
								    {if ($key_a == $VALUE_PARAM)}  
					    		   		<td>= <input type="text" name="{$VALUE_PARAM}" id="value_param{$KEY_PARAM}" value="{$value_a}" class="text"> </td>
					    		   		{assign var="flag_param" value="1"}
								    {/if}
								  {/foreach}
								  {if ($flag_param == 0)}
				    		   		<td>= <input type="text" name="{$VALUE_PARAM}" id="value_param{$KEY_PARAM}" value="" class="text"> </td>				  
								  {/if}	
								  </tr>	      	  
					    		{/foreach}	      	  
				    	      {/if}				
			...
			<tr>
				<td coslpan="2"><input type="hidden" name="parameters" id="parameters" value="{$user_page.parameters}" class="text" /></td>
			</tr>
			<tr>
				<td valign="top">Keywords</td>
				<td><textarea name="keywords" class="text" cols=55 rows=4>{$user_page.keywords}</textarea></td>
			</tr>
			<tr>
				<td valign=top>Description</td>
				<td><textarea name="description" class="text" cols=55 rows=4>{$user_page.description}</textarea></td>
			</tr>
			<tr>
				<td colspan="2"><span class="greenButtonEnd"><input type="submit" value="{if $IS_NEW == 1}Add{else}Update{/if}" class="greenButton" onclick="formTextOfParams()" LANGUAGE="Javascript"></span></td>
			</tr>
		</table>
	</fieldset>
</form>
