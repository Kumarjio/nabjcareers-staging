{breadcrumbs}<a href="{$GLOBALS.site_url}/manage-listings/?restore=1">Manage Listings</a> &#187; <a href="{$GLOBALS.site_url}/edit-listing/?listing_id={$listing_id}">Edit Listing</a> &#187; Listing Comments{/breadcrumbs}
...
<form method="post" name="comments" action="{$GLOBALS.site_url}/listing-comments/">
	...
	<span class="deleteButtonEnd"><input onClick="if(!confirm('Are you sure you want to delete this comments?')) return false; document.forms['comments'].elements['action'].value = 'delete'; return true" value="Delete" class="deleteButton" type="submit"></span>
		...
		<tbody>
			{foreach from=$comments item=comment name=each_comment}
				<tr class="{cycle values = 'evenrow,oddrow'}">
					...
					<td><a href="/admin/edit-user/?username={$comment.user.user_name}">{$comment.user.user_name}</a></td>
					...
					<td><a href="{$GLOBALS.site_url}/listing-comments/?action=edit&amp;comment_id={$comment.id}" title="Edit"><img src="{image}edit.png" border=0 alt="Edit"></a></td>
					<td><a href="{$GLOBALS.site_url}/listing-comments/?action=Delete&amp;comment_id={$comment.id}" onClick="return confirm('Are you sure you want to delete this comment?')" title="Delete"><img src="{image}delete.png" border=0 alt="Delete"></a></td>
				</tr>
			{/foreach}
		</tbody>
	</table>
