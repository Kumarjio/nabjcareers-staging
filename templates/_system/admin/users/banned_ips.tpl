...
<table>
	...
	{foreach from=$bannedIPs item=ip}
		<tr class="{cycle values = 'evenrow,oddrow'}">
			<td>{$ip.value}</td>
			<td><span class="greenButtonEnd"><input type="button" name="button" value="Unban" class="greenButton" onclick="window.location='?id={$ip.id}&action=unban&page={$page}&ip_per_page={$ip_per_page}'" /></span></td>
		</tr>
	{/foreach}
</table>