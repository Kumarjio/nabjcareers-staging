...
<h1>Edit User Profile Fields</h1>
...
{foreach from=$errors key=error item=message}
	...
	{/if}
{foreachelse}
	<p><a href="{$GLOBALS.site_url}/add-user-profile-field/?user_group_sid={$user_group_sid}">Add User Profile Field</a></p>
	<table>
		<thead>
			<tr>
				<th>SID</th>
		...
		{foreach from=$user_profile_fields item=user_profile_field name=fields_block}
			<tr class="{cycle values = 'evenrow,oddrow'}">
				...
				<td><a href="{$GLOBALS.site_url}/edit-user-profile-field/?sid={$user_profile_field.sid}&amp;user_group_sid={$user_group_sid}" title="Edit"><img src="{image}edit.png" border=0 alt="Edit" /></a></td>
				<td><a href="{$GLOBALS.site_url}/delete-user-profile-field/?sid={$user_profile_field.sid}&amp;user_group_sid={$user_group_sid}" onclick="return confirm('Are you sure you want to delete this field?')" title="Delete"><img src="{image}delete.png" border=0 alt="Delete" /></a></td>
				<td>
