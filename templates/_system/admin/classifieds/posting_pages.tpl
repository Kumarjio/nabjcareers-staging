...
<h1>Edit {$listingTypeInfo.name} Posting Pages</h1>

<p><a href="{$GLOBALS.site_url}/posting-pages/{$listingTypeInfo.id|lower}/new">Add a New Posting Page</a></p>
	...
	<tbody>
		{foreach from=$pages item=page name=page_loop}
			<tr class="{cycle values = 'evenrow,oddrow'}">
				...
				<td align="center"><a href="{$GLOBALS.site_url}/posting-pages/{$listingTypeInfo.id|lower}/edit/{$page.sid}" title="Edit"><img src="{image}edit.png" border="0" alt="Edit" /></a></td>
				<td  align="center">
					{if $countPages > 1}			
						<a href="{$GLOBALS.site_url}/posting-pages/{$listingTypeInfo.id|lower}/delete/{$page.sid}" {if $page.fields_num>0}onclick='return confirm("Fields contained on this Page will be removed from the front-end as well, until added to one of the Posting Pages again. Delete this Posting page?")'{else}onclick='return confirm("Are you sure you want to delete this Posting Page?")'{/if} title="Delete"><img src="{image}delete.png" border="0" alt="Delete"/></a>
					{/if}
				</td>
				<td align="center">
