...
<p>
{*	Results: {$count_listings} deleted jobs *}
...
<table>
	<thead>
		...
		{*			
			...
			<th>
				<a href="?restore=1&amp;sorting_field=Title&amp;sorting_order={if $listing_search.sorting_order == 'ASC' && $listing_search.sorting_field == 'Title'}DESC{else}ASC{/if}">Job Title</a>
			...
			<th>
				<a href="?restore=1&amp;sorting_field=activation_date&amp;sorting_order={if $listing_search.sorting_order == 'ASC' && $listing_search.sorting_field == 'activation_date'}DESC{else}ASC{/if}">Posting Date</a>
			...
			<th>
				...
				{/if}
							...
							<th>
								<a href="?restore=1&amp;sorting_field=expiration_date&amp;sorting_order={if $listing_search.sorting_order == 'ASC' && $listing_search.sorting_field == 'expiration_date'}DESC{else}ASC{/if}">Company Name</a>
								{if $listing_search.sorting_field == 'company_name'}
									{if $listing_search.sorting_order == 'ASC'}
										<img src="{image}b_up_arrow.gif" alt="Up" />
									{else}
										<img src="{image}b_down_arrow.gif" alt="Down" />
	...
	<tbody>
		{foreach from=$listings item=listing name=listings_block}			
			{if $listing.deleted == 1 && $listing.type.id == "Job"} {* if statement for 09-06-2014 Deleted jobs mod *}
				<tr class="{cycle values = 'evenrow,oddrow'}">
					...
					<td><span title="{$listing.expiration_date}">{$listing.expiration_date|regex_replace:"/\s.*/":""}</span></td>
							...
							<td><span title="{$listing.CompanyName}">{$listing.CompanyName}</span></td>
			       ...
			       {* <td>
			        ...
			        {* 09-06-2014 Deleted jobs mod *}
			        <td><a href="?restore=restore_job&amp;restore_listing_id={$listing.id}&amp;action=search&listing_type%5Bequal%5D=Job&deleted%5Bequal%5D=1" title="Restore"><img src="{image}restore.png" border=0 alt="Restore"></a></td>
			        {**  END **}
			        
					{*<td><a href="{$GLOBALS.site_url}/edit-listing/?listing_id={$listing.id}" title="Edit"><img src="{image}edit.png" border=0 alt="Edit"></a></td>	*}
					<td><a href="{$GLOBALS.site_url}/listing-actions/?action_name=Delete&amp;listings[{$listing.id}]=1" onclick="return confirm('Are you sure you want to delete permanently this job?')" title="Delete Permanently"><img src="{image}delete.png" border="0" alt="Delete"></a></td>
				</tr>
			{/if} {* 09-06-2014 Deleted jobs mod *}
		{/foreach}
	</tbody>
</table>
