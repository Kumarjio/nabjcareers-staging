{if $type_sid}
	{breadcrumbs}<a href="{$GLOBALS.site_url}/listing-types/">Listing Types</a> &#187; <a href="{$GLOBALS.site_url}/edit-listing-type/?sid={$type_sid}">{$type_info.name}</a>  &#187; <a href="{$GLOBALS.site_url}/edit-listing-type-field/?sid={$field_sid}">{$field_info.caption}</a> &#187; Edit List{/breadcrumbs}
{else}
	{breadcrumbs}<a href="{$GLOBALS.site_url}/listing-fields/">Listing Fields</a>  &#187; <a href="{$GLOBALS.site_url}/edit-listing-field/?sid={$field_sid}">{$field_info.caption}</a> &#187; Edit List{/breadcrumbs}
{/if}
<h1>Edit List</h1>
	...
	<form method="post" action="" id="add_values">
		...
		<table>
			<tr>
				<td>Value </td>
				<td>
					<input name="list_item_value" class="textField" /> <font color="red">*</font>
					<textarea name="list_multiItem_value" style="display:none"></textarea><br/>
				</td>
				<td>
					<span class="greenButtonInEnd"><input type="submit" value="Add" class="greenButtonIn" /></span> <span class="greenButtonInEnd"><input type="button" value="Add multiple values" class="greenButtonIn" onClick="windowMessage();"/></span> 
				</td>
			</tr>
		</table>
	</form>
...
<form method="post" action="" name="list_items_form" id="list_items_form">
	...
	<span class="greenButtonInEnd"><input type="button" id="list_delete_button" value="Delete" class="greenButtonIn" onclick="if ( confirm('Are you sure to delete selected item(s)?') ) submitForm('delete');"></span>
	<div class="clr"><br/></div>
	<table id="list_table">
		...
		<tbody>
			...
			<tr class="{cycle values = 'evenrow,oddrow' advance=true}">
				...
				<td><a href="{$GLOBALS.site_url}/edit-listing-field/edit-list-item/?field_sid={$field_sid}&amp;item_sid={$sid}" title="Edit"><img src="{image}edit.png" border="0" alt="Edit"/></a></td>
