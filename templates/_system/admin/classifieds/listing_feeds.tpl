...
{/foreach}
<p><a href="{$GLOBALS.site_url}/listing-feeds/?action=add">Add a New Feed</a></p>
...
<table>
	...
	<tbody>
		...
		<tr class="{if $counter is odd}oddrow{else}evenrow{/if}">
			...
			<td><a href="{$GLOBALS.site_url}/listing-feeds/?action=edit&amp;feedId={$feed.sid}" title="Edit"><img src="{image}edit.png" border=0 alt="Edit"></a></td>
			<td><a href="{$GLOBALS.site_url}/listing-feeds/?action=delete&amp;feedId={$feed.sid}" onclick="return confirm('Are you sure you want to delete the \'{$feed.name}\' item?')" title="Delete"><img src="{image}delete.png" border=0 alt="Delete"></a></a></td>
		</tr>
	</tbody>
{/foreach}
</table>