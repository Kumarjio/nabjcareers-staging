...
<p><a href="{$GLOBALS.site_url}/alphabet-letters/?action=new">Add New Alphabet</a></p>
<table>
	<thead>
		<tr>
			<th>SID</th>
	...
	<tbody>
		{foreach from=$alphabetInfo item=alphabet name=fields_block}
			<tr class="{cycle values = 'evenrow,oddrow'}">
				...
				<td><a href="{$GLOBALS.site_url}/alphabet-letters/?action=edit&sid={$alphabet.sid}" title="Edit"><img src="{image}edit.png" border=0 alt="Edit"></a></td>
				<td><a href="{$GLOBALS.site_url}/alphabet-letters/?action=delete&sid={$alphabet.sid}" onclick="return confirm('Are you sure you want to delete this alphabet?')" title="Delete"><img src="{image}delete.png" border=0 alt="Delete"></a></td>
				<td>
					{if $smarty.foreach.fields_block.iteration < $smarty.foreach.fields_block.total}
						<a href="?action=move_down&sid={$alphabet.sid}"><img src="{image}b_down_arrow.gif" border=0>
					{/if} 
				</td>
				<td>
					{if $smarty.foreach.fields_block.iteration > 1}
						<a href="?sid={$alphabet.sid}&action=move_up"><img src="{image}b_up_arrow.gif" border=0>
					{/if} 
				</td>
			</tr>
		{/foreach}
