...
<div class="SearchResultsCompany">
	...
	{else}
		{if $tmp_listing.user.CompanyName eq '' }
			<h1>[[Last Name Search Results]]</h1>
		{/if}
	<!-- RESULTS / PER PAGE / NAVIGATION -->
	<div class="topNavBarLeft"></div>
	<div class="topNavBar">
		<div class="numberResults">Results: {$usersCount} users</div>
	...
	<table cellspacing="0">
		<thead>
			<tr>
				...
				<th>
					<a href="?searchId={$searchId}&sorting_field=LastName&sorting_order={if $sorting_order == 'ASC' && $sorting_field == 'LastName'}DESC{else}ASC{/if}">[[Last name]]</a>
					{if $sorting_field == 'LastName'}{if $sorting_order == 'ASC'}<img src="{image}b_up_arrow.png" alt="Up" />{else}<img src="{image}b_down_arrow.png" alt="Down" />{/if}{/if}
				</th>
				<th>
				...
				<th class="tableRight">Actions</th>
			</tr>
		</thead>
		<tbody>
			{foreach from=$found_users_sids item=user_sid name=users_block}
				<tr class="{cycle values = 'evenrow,oddrow' advance=true}">
					...
					{display property='LastName' object_sid=$user_sid assign='companyNameAlias'}
			{*		<td class="compLogo" colspan="2">
 					...
 					<td>
						<strong>
							{if strpos($companyNameAlias, '-') !== false}
								<a href="{$GLOBALS.site_url}/search-results-jobs/?action=search&amp;username[equal]={$user_sid}">{display property='LastName' object_sid=$user_sid}</a>
							{else}
								<a href="{$GLOBALS.site_url}/edit-user/?user_sid={$user_sid}&user_group_id=Employer">{display property='LastName' object_sid=$user_sid}</a>
							{/if}
						</strong>
					</td>
					...
					<td><a href="{$GLOBALS.site_url}/edit-user/?user_sid={$user_sid}&user_group_id=Employer" title="Edit"><img src="{image}edit.png" border=0 alt="Edit"></a></td>
				</tr>
			{/foreach}
		</tbody>
	</table>
