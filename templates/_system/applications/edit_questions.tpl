...
{if $edit}
	<p style="color: green; ">[[Your changes were successfully saved!]]</p>
{/if}
...
<fieldset id="boolean" {if !$answer_boolean}style="display:none"{/if}>
	...
	<div class="inputField">
		<div style="width:50px; float:left;padding-bottom:10px">[[Yes]]&nbsp;<input type="hidden" name="answer_boolean[]" value="Yes" /></div>
		<div>
		<select name="score_boolean[]" class="searchList">
		...
		<div class="clear"></div>
		<div style="width:50px; float:left;padding-bottom:5px">[[No]]&nbsp;<input type="hidden" name="answer_boolean[]" value="No" /></div>
		<div>
		<select name="score_boolean[]" class="searchList">
	...
	<div class="inputField">
		...
		<div id="answerBlock{$key}">
			...
			<div style="float:left;">
				<select name="score[]"  class="searchList">
			...
			<div>&nbsp;&nbsp;<a href='' onclick="deleteAnswerBlock('answerBlock{$key}'); return false;">[[Delete]]</a></div>
		</div>
		<div class="clear"></div>
		{/foreach}
		...
		<div>&nbsp;&nbsp;<a href='' onclick="deleteAnswerBlock('answerBlock'); return false;">[[Delete]]</a></div>
		</div>
		<div class="clear"></div>
...
<table cellspacing="0">
	...
	{foreach from=$questions item=question  name=question_block}
		<tr class="{cycle values = 'evenrow,oddrow'}">
			...
			<td><strong>{$question.caption}</strong></td>
			...
			<td><a href="{$GLOBALS.site_url}/screening-questionnaires/delete-question/{$question.questionnaire_sid}/{$question.sid}" onclick="return confirm('Are you sure you want to delete this question?')">[[Delete]]</a></td>
			<td></td>
		</tr>
		<tr>
...
function addAnswerBlock() {
	...
	$('#'+ id).html($('#'+ id).html() + "<div>&nbsp;&nbsp;<a href='' onclick=\"deleteAnswerBlock('"+id+"'); return false;\">{/literal}[[Delete]]{literal}</a></div><div class='clear'></div>");
	i++;
}
