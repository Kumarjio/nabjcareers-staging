...
<select name='{$id}[monetary][currency]' id='{$id}_list'>
	...
	{foreach from=$list_currency item=list_curr}
		<option value='{$list_curr.currency_code}' {if $list_curr.currency_code == $value.monetary.currency || (!$value.monetary.currency && $list_curr.main==1)}selected="selected"{/if} >{tr mode="raw" domain="Property_$id"}{$list_curr.currency_sign}{/tr}</option>
	{/foreach}
</select>