...
ul.tree li .arrow {
	...
	background-image: url({/literal}{$GLOBALS.site_url}{literal}/system/ext/jquery/ltL_nes.gif);
}
...
<input type='hidden' name='{$id}[tree]' id='tree_{$id}_selected' value='' />
...
<div id="tree_div_{$id}" class="select-free-fix">
	<div id="div_content_{$id}" class="bd">[[Please wait ...]] <img src='{$GLOBALS.user_site_url}/system/ext/jquery/progbar.gif' alt="[[Please wait ...]]" /></div>
...
$.get("{$GLOBALS.user_site_url}/get-tree/", {ldelim} id: "{$sid}", name: "{$id}", check: tree_{$id}_select_string, search: "yes" {rdelim},
            ...
            $(this).children("li").each(function(li){
                ...
                $(this).children(":checkbox").attr("checked", act);
                    if ($(this).children("ul").size() > 0){
                        ...
                        setChildrenStatus_{/literal}{$id}(new_my_id, act);{literal}
                    }
            });
     });
}
