...
<table cellspacing="0" style="width: 60%;">
	<thead>
		<tr>
	...
	{foreach from=$saved_searches item=saved_search}
		<tr>
			...
			<td><a href="?action=delete&amp;search_id={$saved_search.id}" onclick="return confirm('Are you sure to delete \'{$saved_search.name}\' search?');">[[Delete]]</a></td>
			<td>
				<form style="margin:0;padding:0;" method="post" action="{$GLOBALS.site_url}/saved-searches/" id="editForm_{$saved_search.id}">
			...
			<td>
				<form style="margin:0;padding:0;" method="post" action="{$GLOBALS.site_url}/{if $saved_search.listing_type == 'Resume'}search-results-resumes/{else}search-results-jobs/{/if}">
					...
					{foreach from=$saved_search.data item=criteria_fields}
						...
						{/foreach}
					{/foreach}
					<input type="submit" value="[[Go search:raw]]" class="button" />
				</form>
			</td>
			<td> </td>
		</tr>
