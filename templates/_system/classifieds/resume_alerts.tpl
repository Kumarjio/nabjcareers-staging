<h1>{if $action == 'save'}[[Add resume alert]]{elseif $action == 'edit'}[[Edit resume alert]]{else}[[Resume Alerts]]{/if}</h1>
...
{if $action == 'edit'}<input type="hidden" name="id_saved" value="{$id_saved}" />{/if}
	...
	<fieldset>
		...
		<div class="inputField">{search property=DesiredSalary} {search property=DesiredSalaryType}</div>
	</fieldset>
	<fieldset>
...
<br /><br />
{else}
<p><a href="?action=new">[[Add new resume alert]]</a></p>
<table cellspacing="0" style="width: 70%;">
	...
	{foreach from=$saved_searches item=saved_search}
		<tr class="{cycle values = 'evenrow,oddrow' advance=true}">
			...
			<td width="10%">
				<form style="margin:0;padding:0;" method="post" action="{$GLOBALS.site_url}/resume-alerts/" id="editForm_{$saved_search.id}">
