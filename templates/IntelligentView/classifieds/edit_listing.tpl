...
{else}
	...
	{if $form_is_submitted && !$errors && !$field_errors}
		...
		{literal}
		<script language="JavaScript">location.href='{/literal}{$GLOBALS.site_url}/manage-listing/?listing_id={$listing.id}{if $GLOBALS.user_page_uri == "/edit-listing/" && $listing.type.id =="Job"}&edit_job=edit_job{/if}{literal}';</script>
		{/literal}
	{/if}
